...
contract FeeProcessor is Ownable, ReentrancyGuard, IFeeProcessor {
    ...
    function addBridgeToken(IERC20 token, IERC20 bridgeToken) external override onlyOwner {
        require(token != GRM, "FeeProcessor: INVALID_TOKEN_GRM");
    ...
    function processBorrowable(IBorrowable borrowable, address to) public override nonReentrant {
        ...
        // GRM balance before any redemption
        uint256 gremlinBalanceBefore = GRM.balanceOf(address(this));
...

        {
            ...
            // Convert whatever we received to GRM
            _toGRM(token, tokenAmount);
        }

        uint256 gremlinBalanceAfter = GRM.balanceOf(address(this));
        // Net amount to use for reward
        uint256 gremlinBalanceChange = gremlinBalanceAfter.sub(gremlinBalanceBefore);
        // Now have GRM so send reward to caller and rest to fee distributor
        ...
        if (rewardAmount > 0) {
            GRM.safeTransfer(to, rewardAmount);
        }
        uint256 gremlinAmount = gremlinBalanceAfter.sub(rewardAmount);
        GRM.safeTransfer(address(m_feeDistributor), gremlinAmount);

        emit ProcessBorrowable(msg.sender, address(borrowable), gremlinAmount, rewardAmount);
    }

    function _toGRM(IERC20 token, uint256 amount) private returns (uint256 gremlinOut) {
        if (token == GRM) {
            // We have GRM so no-op
            gremlinOut = amount;
        } else {
            IERC20 bridgeToken = getBridgeToken(token);
            if (bridgeToken == GRM) {
                // Swap for GRM and we are done
                gremlinOut = _doOptimalSwap(token, GRM, amount, address(this));
            } else {
                ...
                gremlinOut = _toGRM(bridgeToken, bridgeAmount);
            }
        }
    }
