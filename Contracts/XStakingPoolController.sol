...
contract XStakingPoolController is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 xGRMStakedAmount; // # of xGRM allocated to this pool
    ...
    constructor(IERC20 _xGRM)public {
        xGRM = _xGRM;
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.xGRMStakedAmount != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp, pool);
            uint256 reward = multiplier * pool.RewardPerSecond;
            accRewardPerShare += (reward * pool.TokenPrecision) / pool.xGRMStakedAmount;
        }
    ...
    function updatePool(uint256 _pid) internal {
        ...
        pool.accRewardPerShare += reward * pool.TokenPrecision / pool.xGRMStakedAmount;
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {  
        ...
        safeTransfer(xGRM, address(msg.sender), _amount);
    ...
    function emergencyWithdraw(uint256 _pid) external {
        ...
        pool.xGRMStakedAmount -= user.amount;
        user.amount = 0;
        user.rewardDebt = 0;

        xGRM.safeTransfer(address(msg.sender), oldUserAmount);
    ...
    function recoverWrongTokens(address _tokenAddress) external onlyOwner {
        require(_tokenAddress != address(xGRM), "recoverWrongTokens: Cannot be xGRM");
